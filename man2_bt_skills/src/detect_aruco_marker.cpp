...
void DetectArucoMarker::getMarkerCallback(const aruco_msgs::msg::MarkerArray & msg)
{
  if (online_.load()) {
    ...
    // RCLCPP_INFO(node_->get_logger(), "MarkerArray: %s",
    // aruco_msgs::msg::to_yaml(msg).c_str()); Find marker with our ID
...
BT::NodeStatus DetectArucoMarker::onRunning() { RCLCPP_INFO(node_->get_logger(), "on running"); }
...
// PLUGINLIB_EXPORT_CLASS(perception_skills::DetectArucoMarker(const
