...
#include "man2_bt_skill_clients/action_clients/compute_path_to_state_action_bt_client.hpp"

namespace man2_bt_skill_clients
{
void ComputePathToStateActionClient::on_tick()
{
  {
  ...
  auto res = getInput<std::string>("named_state");
  if (!res) {
    throw BT::RuntimeError("error reading port [named_state]:", res.error());
  } else {
    goal_.named_state = res.value();
  }
  ...
  RCLCPP_INFO(
    *logger_, "target_group: %s, named_state:%s, planning_pipeline: %s, planner_id: %s\n",
    goal_.target_group.c_str(), goal_.named_state.c_str(), goal_.config.planning_pipeline.c_str(),
    goal_.config.planner_id.c_str());
}

BT::NodeStatus ComputePathToStateActionClient::on_success()
{
...
BT_REGISTER_NODES(factory)
{
  factory.registerBuilder(
    BT::CreateManifest<man2_bt_skill_clients::ComputePathToStateActionClient>(
      "ComputePathToState", man2_bt_skill_clients::ComputePathToStateActionClient::providedPorts()),
    BT::CreateBuilder<man2_bt_skill_clients::ComputePathToStateActionClient>());
}
