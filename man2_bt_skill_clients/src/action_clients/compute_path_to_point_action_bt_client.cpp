...
#include "man2_bt_skill_clients/action_clients/compute_path_to_point_action_bt_client.hpp"

namespace man2_bt_skill_clients
{
void ComputePathToPointActionClient::on_tick()
{
  {
  ...
  auto res = getInput<geometry_msgs::msg::PointStamped>("target_point");
  if (!res) {
    throw BT::RuntimeError("error reading port [target_point]:", res.error());
  } else {
    goal_.target_point = res.value();
  }
  ...
  RCLCPP_INFO(
    *logger_, "target_group: %s, target_point:%s, planning_pipeline: %s, planner_id: %s\n",
    goal_.target_group.c_str(), geometry_msgs::msg::to_yaml(goal_.target_point).c_str(),
    goal_.config.planning_pipeline.c_str(), goal_.config.planner_id.c_str());
}

BT::NodeStatus ComputePathToPointActionClient::on_success()
{
...
BT_REGISTER_NODES(factory)
{
  factory.registerBuilder(
    BT::CreateManifest<man2_bt_skill_clients::ComputePathToPointActionClient>(
      "ComputePathToPoint", man2_bt_skill_clients::ComputePathToPointActionClient::providedPorts()),
    BT::CreateBuilder<man2_bt_skill_clients::ComputePathToPointActionClient>());
}
