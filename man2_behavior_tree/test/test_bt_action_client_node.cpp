...
/**
 * @brief test bt action client node
 *
 */
class FibonacciAction : public ros2_behavior_tree::BtActionClientNode<test_msgs::action::Fibonacci>
{
public:
  FibonacciAction(const std::string & xml_tag_name, const BT::NodeConfiguration & conf)
  : ros2_behavior_tree::BtActionClientNode<test_msgs::action::Fibonacci>(
      xml_tag_name, "fibonacci", conf)
  {
  }
...
public:
  ...
  {
    node_ = std::make_shared<rclcpp::Node>("bt_action_client_node_test");
    ...
    factory_->registerBuilder(
      BT::CreateManifest<FibonacciAction>("Fibonacci", FibonacciAction::providedPorts()),
      BT::CreateBuilder<FibonacciAction>());
  }
