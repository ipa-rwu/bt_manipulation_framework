...
#ifndef MAN2_BEHAVIOR_TREE__BT_SERVICE_CLIENT_NODE_HPP_
#define MAN2_BEHAVIOR_TREE__BT_SERVICE_CLIENT_NODE_HPP_
...
/**
 * @brief Abstract class representing a service client based used as BT node
 * @tparam ServiceT Type of service
 */
template <class ServiceT>
class BtServiceClientNode : public BT::ActionNodeBase
{
public:
  /**
   * @brief A ros2_behavior_tree::BtServiceClientNode constructor
    ...
    service_node_name_(service_node_name)
  {
    ...
    service_client_ =
      node_->create_client<ServiceT>(service_name_, rmw_qos_profile_default, callback_group_);

    // Make a request for the service without parameter
    request_ = std::make_shared<typename ServiceT::Request>();

    std::string node_name = node_->get_name();
    logger_ = std::make_shared<rclcpp::Logger>(rclcpp::get_logger(node_name + service_name_));

    // Make sure the server is actually there before continuing
    RCLCPP_DEBUG(*logger_, "Waiting for \"%s\" service", service_name_.c_str());
    if (!service_client_->wait_for_service(1s)) {
      RCLCPP_ERROR(
        *logger_, "\"%s\" service server not available after waiting for 1 s",
        service_node_name.c_str());
      throw std::runtime_error(
        std::string("Service server %s not available", service_node_name.c_str()));
    }

    RCLCPP_DEBUG(*logger_, "\"%s\" BtServiceClientNode initialized", service_node_name_.c_str());
  }

  BtServiceClientNode() = delete;

  virtual ~BtServiceClientNode() {}

  /**
   * @brief Any subclass of BtServiceClientNode that accepts parameters must provide a
  ...
  virtual BT::NodeStatus check_future()
  {
    ...
    RCLCPP_WARN(
      *logger_, "Node timed out while executing service call to %s.", service_name_.c_str());
    request_sent_ = false;
    return BT::NodeStatus::FAILURE;
  }
...
#endif  //MAN2_BEHAVIOR_TREE__BT_SERVICE_CLIENT_NODE_HPP_
