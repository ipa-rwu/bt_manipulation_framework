...
#ifndef MAN2_BEHAVIOR_TREE__BT_ACTION_CLIENT_NODE_HPP_
#define MAN2_BEHAVIOR_TREE__BT_ACTION_CLIENT_NODE_HPP_
...
{
/**
 * @brief Abstract class representing a ros action client used as BT node
 *
 */
...
public:
  ...
  : BT::ActionNodeBase(xml_tag_name, conf), action_name_(action_name), should_send_goal_(true)
  {
    ...
    logger_ = std::make_shared<rclcpp::Logger>(rclcpp::get_logger(node_name + action_name_));

    createActionClient(action_name_);

    // Give the derive class a chance to do any initialization
    RCLCPP_DEBUG(*logger_, "\"%s\" BtActionClientNode initialized", xml_tag_name.c_str());
  }
  ...
  void createActionClient(const std::string & action_name)
  {
    ...
    RCLCPP_DEBUG(*logger_, "Waiting for \"%s\" action server", action_name.c_str());
    if (!action_client_->wait_for_action_server(1s)) {
      RCLCPP_ERROR(
        *logger_, "\"%s\" action server not available after waiting for 1 s", action_name.c_str());
  ...
  /**
   * @brief Any subclass of BtActionClientNode that accepts parameters must provide a
  ...
  BT::NodeStatus tick() override
  {
    ...
    switch (result_.code) {
      ...
      default:
        throw std::logic_error("BtActionClientNode::Tick: invalid status value");
    }
  ...
  void halt() override
  {
    if (should_cancel_goal()) {
      ...
      if (
        ...
        RCLCPP_ERROR(*logger_, "Failed to cancel action server for %s", action_name_.c_str());
      }
    }
...
#endif  //MAN2_BEHAVIOR_TREE__BT_ACTION_CLIENT_NODE_HPP_
